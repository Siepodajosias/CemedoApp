...
import { ComptableService } from 'src/app/_services/comptable/comptable.service';
...
export class ComptableComponent implements OnInit {
  ...
  constructor(public dialogService: DialogService,private service: ComptableService,private messageService: MessageService, private confirmationService: ConfirmationService) { }
  ngOnInit() {
    this.cols = [
  ...
  editProduct(specialite: Comptable) {
    this.specialite = {...specialite};
    this.entityDialog = true;
  }
  
  deleteProduct(specialite: Comptable) {
    this.confirmationService.confirm({
        message: 'Are you sure you want to delete ' + specialite.nom + '?',
        header: 'Confirm',
        icon: 'pi pi-exclamation-triangle',
        accept: () => {
