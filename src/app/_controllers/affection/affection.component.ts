...
import { AffectionService } from 'src/app/_services/affection/affection.service';
...
export class AffectionComponent implements OnInit {
  ...
  liste: Affection[] ;
  specialite: Affection;
  entityDialog: boolean;
  totalRecords: number;
  loading: boolean;
  selectedEntity: Affection[];
  submitted: boolean;
  ...
  constructor(public dialogService: DialogService,private service: AffectionService,private messageService: MessageService, private confirmationService: ConfirmationService) { }
  ngOnInit() {
    this.cols = [
  ...
  editProduct(specialite: Affection) {
    this.specialite = {...specialite};
    this.entityDialog = true;
  }
  
  deleteProduct(specialite: Affection) {
    this.confirmationService.confirm({
