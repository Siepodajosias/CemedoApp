...
@Component({
  ...
  styleUrls: ['./type-employe.component.scss'],
  providers: [MessageService,ConfirmationService,DialogService]
})
export class TypeEmployeComponent implements OnInit {
  liste: TypeEmploye[] ;
  specialite: TypeEmploye;
  entityDialog: boolean;
  totalRecords: number;
  loading: boolean;
  selectedEntity: TypeEmploye[];
  submitted: boolean;
  ...
  constructor(public dialogService: DialogService,private typeEmployeService: TypeEmployeService,private messageService: MessageService, private confirmationService: ConfirmationService) { }
  ngOnInit() {
    this.cols = [
    ...
    this.typeEmployeService.getListe().then(
  ...
  editProduct(specialite: TypeEmploye) {
    this.specialite = {...specialite};
    this.entityDialog = true;
  }
  
  deleteProduct(specialite: TypeEmploye) {
    this.confirmationService.confirm({
